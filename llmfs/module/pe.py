...
class AbsolutePositionalEncoding(nn.Module):
    ...
    def __init__(
        self, d_model: int, max_len: int = 5000, n: int = 10000, dropout: float = 0.1
    ) -> None:
        """
        P(k, 2i)   = sin(k / n^{2i/d}),
        ...
        div = torch.exp(
            torch.arange(0, d_model, 2, dtype=torch.float) * (-math.log(n) / d_model)
        )  # (d_model/2)
...
def precompute_rot_matrix(d_model: int, max_len: int, n: int = 10000) -> torch.Tensor:
    ...
    theta = torch.exp(
        torch.arange(0, d_model - 1, 2, dtype=torch.float) * (-math.log(n) / d_model)
    )  # (d_model/2)
...
def apply_rope(
    xq: torch.Tensor, xk: torch.Tensor, rot_matrix: torch.Tensor
    ...
    )  # (bsz, seqlen, num_heads, head_dim/2, 2)
    ...
    xq_out = torch.view_as_real(xq_ * rot_matrix[..., : xq.size(2), :]).flatten(
        3
    )  # (bsz, seqlen, num_heads, head_dim)
    xk_out = torch.view_as_real(xk_ * rot_matrix[..., : xq.size(2), :]).flatten(
        3
    )  # (bsz, seqlen, num_heads, head_dim)
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def __init__(
        self, d_model: int, max_len: int, n: int = 10000, dropout: float = 0.1
    ) -> None:
        """
        ...
        theta = torch.exp(
            torch.arange(0, d_model - 1, 2, dtype=torch.float)
            * (-math.log(n) / d_model)
        )  # [d_model/2]
        ...
        self.register_buffer(
            "rope", rope
        )  # be aware that rope embeds real part (cos) and imaginary (sin) part
